...
def send_sport_news(message):
    """
    Эта функция получает последние новости о спорте с указанного URL и отправляет
    массив из первых 5 заголовков новостей вместе с их ссылками в функцию select_new().
    """
    url = 'https://rsport.ria.ru/'  # URL страницы с новостями спорта
    base_url = 'https://rsport.ria.ru'  # Базовый URL для формирования полных ссылок
    ...
    if response.status_code == 200:
        ...
        news_items = soup.find_all('div', class_='cell-list__item')
        ...
        if news_items:
            ...
            for news in news_items[:5]:
                title_tag = news.find('span', class_='cell-list__item-title')
                link_tag = news.find('a', class_='cell-list__item-link')
                if title_tag and link_tag:
                    ...
                    news_array.append({'topic': 'Спорт', 'title': title, 'link': absolute_link})
